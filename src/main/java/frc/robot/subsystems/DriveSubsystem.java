...
public class DriveSubsystem extends SubsystemBase{
    ...
    private final MAXSwereveModule kFLeft = new MAXSwereveModule(DriveConstants.kFrontLeftSteerCANID, DriveConstants.kFrontLeftDriveCANID, DriveConstants.kFrontLeftOffset);
    private final MAXSwereveModule kFRight = new MAXSwereveModule(DriveConstants.kFrontRightSteerCANID, DriveConstants.kFrontRightDriveCANID, DriveConstants.kFrontRightOffset);
    private final MAXSwereveModule kBLeft = new MAXSwereveModule(DriveConstants.kBackLeftSteerCANID, DriveConstants.kBackLeftDriveCANID, DriveConstants.kBackLeftOffset);
    private final MAXSwereveModule kBRight = new MAXSwereveModule(DriveConstants.kBackRightSteerCANID, DriveConstants.kBackRightDriveCANID, DriveConstants.kBackRightOffset);
    ...
    SwerveDriveOdometry odometry = new SwerveDriveOdometry(DriveConstants.kDriveKinematics, Rotation2d.fromDegrees(-gyro.getAngle()), new SwerveModulePosition[] {
        kFLeft.getPosition(), kFRight.getPosition(), kBLeft.getPosition(), kBRight.getPosition()
    });
    ...
    // controls the robot's movement by setting desired velocity, which is respective to field relative and also responsive to limits of the rate of change
    public void drive(double x, double y, double rotation, boolean fieldrelative, boolean ratelimit) {
        ...
        if(ratelimit) {
            ...
            if (anglediff < 0.45*Math.PI) {
                ...
                currentDirection = SwerveUtils.StepTowardsCircular(currentDirection, inputdirection, directionrate * deltatime);
            ...
            // calculates the limited version of speeds based on the current magnitude and direction
        ...
        double finalx = limitedx * DriveConstants.kMaxSpeed;
        double finaly = limitedy * DriveConstants.kMaxSpeed;
        double finalrot = currentRotation * DriveConstants.kMaxAngSpeed;
        ...
        // checks and makes sure none of the module states exceeds the max allowed speed. 
